...
LinkedList flatten(Node *root)
{
    ...
    // Base case: If the tree is empty, return NULL for head and tail
    if (root == NULL)
    {
        l.head = l.tail = NULL;
        return l;
    ...
    // Case: Only left child is not NULL
    if (root->left != NULL && root->right == NULL)
    {
        LinkedList leftLL = flatten(root->left);
        leftLL.tail->right = root;
        root->left = NULL; // Set left child of current node to NULL to maintain linked list structure
    ...
    // Case: Only right child is not NULL
    if (root->left == NULL && root->right != NULL)
    {
    ...
    // Case: Both left and right children are not NULL
    LinkedList leftLL = flatten(root->left);
    LinkedList rightLL = flatten(root->right);
